...
class CIFPParser:
    ...
    def __init__(self, input_dir, output_dir, debug=False, no_filter=False, manual_data=None):
        """
                        ...
                        try:
                            if line and line[0] in ['2', '3', '4', '5', '6', '12', '13']:
                                ...
                                if len(parts) >= 9:
                                    ...
                                    if self.debug and count <= 5:
                                        logger.debug(f"Пример навигационного средства: {name} - Тип: {type_name}, Lat: {lat}, Lon: {lon}, Частота: {frequency}")
                        except Exception as e:
...
def main():
    ...
    # Запуск парсера
    parser = CIFPParser(args.input, args.output, args.debug, args.no_filter, args.manual)
    parser.run()
